...
class _CustomerManagementPageState extends State<CustomerManagementPage> {
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    _buildStatItem('Total Spent', '\$${stats['totalSpent'].toStringAsFixed(0)}', 
                        Icons.attach_money, Colors.green),
                    _buildStatItem('Last Order', lastOrderFormatted, 
  ...
  Widget _buildStatItem(String label, String value, IconData icon, Color color) {
    return Column(
      children: [
        Icon(icon, color: color, size: 20),
        SizedBox(height: 4),
        Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.grey.shade800)),
  ...
  Future<void> _makeAdmin(String customerId) async {
    ...
    if (confirm == true) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('User has been granted admin privileges'), backgroundColor: Colors.green),
        );
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error updating user role'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Future<void> _disableAccount(String customerId) async {
    ...
    if (confirm == true) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Account has been disabled'), backgroundColor: Colors.orange),
        );
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error disabling account'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  void _exportCustomers() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Exporting customers...'), backgroundColor: Colors.blue),
    );
  }
}

// Customer Details Sheet remains the same
class CustomerDetailsSheet extends StatelessWidget {
  final String customerId;
  final Map<String, dynamic> customerData;
  final Map<String, dynamic> stats;
                  ...
                  Center(
                    child: Column(
                      children: [
                        ...
                        SizedBox(height: 16),
                        Text(customerData['name'] ?? 'Unknown', 
                            style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.grey.shade800)),
                        SizedBox(height: 4),
                        Text(customerData['email'] ?? 'No email', 
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.grey.shade800)),
          ),
        ],
      ),
    );
